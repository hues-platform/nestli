...
class Manager:
    ...
    def initialize_simulators(self):
        ...
        for sim, attributes in self.cfg["SIMULATORS"].items():
            if attributes["TYPE"] == "FMU":
                simulators[attributes["NAME"]] = self.world.start(attributes["TYPE"], work_dir=attributes["PATH"], fmu_name=attributes["NAME"], model_name=attributes["NAME"], instance_name=attributes["TYPE"] + "_Instance", stop_time=self.cfg["END"])
            elif attributes["TYPE"] == "CSV":
                simulators[attributes["NAME"]] = self.world.start(attributes["TYPE"], sim_start=self.cfg["START"], datafile=attributes["PATH"])
            else:
    ...
    def connect_signals(self):
        for mapping, attributes in self.cfg["MAPPINGS"].items():
            ...
            cyclic_dependency = attributes["FROM"] == "UMAR" and attributes["TO"] == "PREPROCESS"
            if cyclic_dependency:
                for key, value in mapping_dict.items():
                    self.world.connect(signal_source, signal_drain, (value, key), time_shifted=True, initial_data={value:0.0})
            else:
                for key, value in mapping_dict.items():
